...
// startup and every six hours, and tells the desktop about plugins that newly
// have an update; the backend's lock makes the widgets on other monitors wait
// for it and then find the cache fresh, so there is one check and one
// notification however many bars there are.
BarWidget {
  id: root
  ...
  Process {
    id: checkProc
    command: [root.pluginDir + "/bin/plugin-manager", "check", "--if-stale", String(root.staleSeconds), "--notify"]
